...
// Set request timeout to 5 minutes (300 seconds)
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      cb(
        new Error("Invalid file type. Supported formats: MP4, MOV, AVI, MKV"),
        false,
      );
      return;
    }
...
app.post("/api/users/:id/subscribe", authMiddleware, async (req, res) => {
  try {
    ...
    const subscribedIndex = currentUser.subscribedTo.indexOf(
      userToSubscribe._id,
    );
